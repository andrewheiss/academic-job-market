...
# Load jobs data
jobs_clean <- read_csv(here("data", "jobs_clean.csv")) %>% 
  mutate_at(vars(`Skype interview`, `Flyout`, contains("ffer")), 
            funs(bin = !is.na(.)))

# Convert lat/long to sf-based geometry
jobs_geo <- jobs_clean %>% 
  mutate_at(vars(Lat, Long), funs(round(., digits = 3))) %>% 
  st_as_sf(coords = c("Long", "Lat"), crs = 4326)

# Maps from Natural Earth
earth_lores <- read_sf(here("data", "ne_110m_admin_0_countries", 
                            "ne_110m_admin_0_countries.shp")) %>% 
  filter(ISO_A3 != "ATA")

earth_medres <- read_sf(here("data", "ne_50m_admin_0_countries", 
                             "ne_50m_admin_0_countries.shp")) %>% 
  filter(ISO_A3 != "ATA")

earth_medres_lakes <- read_sf(here("data", "ne_50m_lakes", "ne_50m_lakes.shp"))
...
earth_medres_states <- read_sf(here("data", "ne_50m_admin_1_states_provinces",
...
Between August 31, 2016 and November 18, 2018, I applied for `r total_jobs` tenure-track and non-tenure-track academic jobs at R1 schools, liberal arts colleges, and teaching-focused public universities. I was offered one two-year visiting assistant professorship at the Romney Institute of Public Service and Ethics at BYU (where I completed my MPA before getting my PhD), and one tenure-track assistant professorship at the Andrew Young School at Georgia State University. 

That's it. That's a `r percent(rejections / total_jobs)` rejection rate. Here's what the weight of rejection looks like:
...
ggsave(reject_waffle, filename = here("output", "reject-waffle.pdf"),
       width = 7, height = 5, units = "in", device = cairo_pdf)
ggsave(reject_waffle, filename = here("output", "reject-waffle.png"),
...
ggsave(plot_disc_props, filename = here("output", "disc-props.pdf"),
       width = 7, height = 2.5, units = "in", device = cairo_pdf)
ggsave(plot_disc_props, filename = here("output", "disc-props.png"),
       width = 7, height = 2.5, units = "in", dpi = 300, type = "cairo")
```

Beyond knowing the division of proportions, it's neat to look at how successful I was across each of these disciplines in each year. This waffle plot divides up each of the applications by their final outcome (i.e. Skype, flyout, offer), discipline, and year. smh polsci.
...
```{r iron-waffles, fig.width=7, fig.height=10}
iron(waffle_ps_2016, waffle_pa_2016,
     waffle_ps_2017, waffle_pa_2017,
     waffle_ps_2018, waffle_pa_2018)

# Have to use png() because iron() doesn't work with ggsave
# https://github.com/hrbrmstr/waffle/issues/54
png(here("output", "disc-cycle-waffles.png"),
    width = 7, height = 10, units = "in", res = 300, type = "cairo", bg = "white")
iron(waffle_ps_2016, waffle_pa_2016,
     waffle_ps_2017, waffle_pa_2017,
     waffle_ps_2018, waffle_pa_2018)
invisible(dev.off())

cairo_pdf(here("output", "disc-cycle-waffles.pdf"), width = 7, height = 10)
iron(waffle_ps_2016, waffle_pa_2016,
     waffle_ps_2017, waffle_pa_2017,
     waffle_ps_2018, waffle_pa_2018)
invisible(dev.off())
...
Writing cover letters generally took ≈45 minutes per school to scour each department's website for relevant programs, centers, faculty, and classes. On some of the days where I sent out 5+ applications, I occasionally forgot to change the recipient address in the letter, or the "I am applying for job X and univeristy Y", leaving "University Z" from the previous application. This was always horrifying, but I got Skype interviews out of a couple of those, so I think search committees tend to be forgiving about those kinds of mistakes.

This plot shows my pace of applying for jobs. Each dot is a job; each column is a week. The fall semester has been the most intense for sending out applications. In 2016–17, my record was 16 jobs in one week; in 2017–18 it was only 6 (since I severly cut back on political science jobs); and in 2018–19, I applied for 30 jobs in one week. With a rough average of 1 hour per application, [that week was particularly intense](https://twitter.com/andrewheiss/status/1046608912617680896).

```{r dotplot-faux-facet, fig.width=6, fig.height=8.5}
...
ggsave(all_dots, filename = here("output", "dotplot-faux-facet.pdf"),
       width = 6, height = 8.5, units = "in", device = cairo_pdf)
ggsave(all_dots, filename = here("output", "dotplot-faux-facet.png"),
       width = 6, height = 8.5, units = "in", dpi = 300, type = "cairo")
```
...
```{r countries-bar, fig.width=7, fig.height=3.5}
...
ggsave(countries_applied, filename = here("output", "countries-applied.pdf"),
       width = 7, height = 3.5, units = "in", device = cairo_pdf)
ggsave(countries_applied, filename = here("output", "countries-applied.png"),
       width = 7, height = 3.5, units = "in", dpi = 300, type = "cairo")
```
...
```{r countries-map-cycle, fig.width=7, fig.height=4}
...
```{r map-blobs, fig.width=8, fig.height=6}
...
ggsave(fancy_map, filename = here("output", "fancy-map.pdf"),
       width = 8, height = 6, units = "in", device = cairo_pdf)
ggsave(fancy_map, filename = here("output", "fancy-map.png"),
       width = 8, height = 6, units = "in", dpi = 300, type = "cairo")
```
